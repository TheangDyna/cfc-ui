...
const MoreButtonClass = ({ id, path, update }) => {
  ...
  const handleClick = async (action, id) => {
    if (action == "Favorite") {
      ...
      if (deleteCover) {
        await deleteDataFunc(`${process.env.NEXT_PUBLIC_API_URL}/students/${id}`);
      }
    }
    handleCloseUserMenu();
  };
...
export default MoreButtonClass;

MoreButtonClass.propTypes = {};

MoreButtonClass.defaultProps = {};
