...
const MoreButtonStory = ({ id, paths, update }) => {
  ...
  const handleClick = async (action, id) => {
    if (action == "Favorite") {
      ...
      if (deleteCover) {
        await deleteDataFunc(`${process.env.NEXT_PUBLIC_API_URL}/stories/${id}`);
      }
    }
    handleCloseUserMenu();
  };
...
export default MoreButtonStory;

MoreButtonStory.propTypes = {};

MoreButtonStory.defaultProps = {};
