...
const PostStoryCard = ({ user, update, setOpen }) => {
    ...
    try {
      update.status
        ? await updateDataFunc(
            `${process.env.NEXT_PUBLIC_API_URL}/stories/${update._id}`,
            body
          )
        : await postDataFunc(
      ...
      <Card className={classes.card}>
        <Box
          ...
          <Typography variant="primary">
            {update.status ? "Update Story" : "Create Story"}
          </Typography>
          <Box flex={1} />
...
export default PostStoryCard;

PostStoryCard.propTypes = {};

PostStoryCard.defaultProps = {};
