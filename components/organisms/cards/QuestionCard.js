import React, { useState } from "react";
import PropTypes from "prop-types";
import {
  Avatar,
  Box,
  IconButton,
  Stack,
  Tooltip,
  Typography,
} from "@mui/material";
import { makeStyles } from "@mui/styles";
import VoteButton from "../../molecules/button/VoteButton";
import HelpRoundedIcon from "@mui/icons-material/HelpRounded";
import BookmarkRoundedIcon from "@mui/icons-material/BookmarkRounded";

const UseStyles = makeStyles((theme) => ({
  root: {
    width: "100%",
  },
  question: {
    flex: 1,
  },
  profileBox: {
    alignItems: "center",
  },
  iconText: {
    color: "#ff9900",
    fontSize: 20,
  },
}));

const QuestionCard = ({
  firstName,
  lastName,
  date,
  profile,
  title,
  description,
  vote,
  answer,
  addVote,
  removeVote,
  userId
}) => {
  const classes = UseStyles();
  return (
    <Box className={classes.root}>
      <Stack
        direction={{ xs: "column", sm: "row" }}
        spacing={{ xs: 0, sm: "20px" }}>
        {/* question */}
        <Box className={classes.question} order={{ xs: 2, sm: 1 }}>
          <Box>
            <HelpRoundedIcon
              className={classes.iconText}
              sx={{ mr: { xs: "5px", sm: "10px" } }}
            />
            <Typography variant="title">{title}</Typography>
          </Box>

          {/* infomation */}
          <Box>
            <Typography variant="small">{vote.length} votes</Typography>
            <Typography variant="secondary"> • </Typography>
            <Typography variant="small">{answer} answers</Typography>
          </Box>
          <Box sx={{ mt: "20px", maxWidth: "800px" }}>
            <Typography variant="secondary">{description}</Typography>
          </Box>
        </Box>
        <Stack
          spacing={{ xs: 0, sm: "20px" }}
          direction={{ xs: "row", sm: "column" }}
          sx={{
            pb: { xs: "20px", sm: 0 },
            alignItems: { xs: "start", sm: "end" },
          }}
          order={{ xs: 1, sm: 2 }}>
          {/*vote button*/}
          <Box order={{ xs: 3, sm: 1 }}>
            <Stack spacing="10px">
              <Tooltip title="save">
                <IconButton>
                  <BookmarkRoundedIcon />
                </IconButton>
              </Tooltip>
              <VoteButton vote={vote} userId={userId} addVote={addVote} removeVote={removeVote} />
            </Stack>
          </Box>
          <Box
            flex={1}
            order={2}
            sx={{ display: { xs: "flex", sm: "none" } }}
          />
          {/* profile */}
          <Stack
            spacing="20px"
            className={classes.profileBox}
            order={{ xs: 1, sm: 3 }}>
            <Typography variant="small">{date}</Typography>
            <Avatar src={profile} />
            <Typography variant="small">
              {firstName} {lastName}
            </Typography>
          </Stack>
        </Stack>
      </Stack>
    </Box>
  );
};

export default QuestionCard;

QuestionCard.propTypes = {};

QuestionCard.defaultProps = {};
